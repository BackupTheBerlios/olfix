...
#**********************************
# Namn: setup.sh
# Version: 0.5.3	2007-02-13 Jan Pihlgren

# Namn: install.sh	(2007-02-13 bytt namn  till setuo.sh)
# Version: 0.4.1	2006-03-12 Jan Pihlgren.
...
# En log skapas vid installationen ($HOME/tmp/SetupOlfix.log)
LOGFILE=$HOME/tmp/SetupOlfix.log
echo "START!" >$LOGFILE
date>>$LOGFILE
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# setup.sh anropas när man står i $OLFIX_HOME/script
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++#
# -- Test om vilken libmysqlclient.so.* som finns. -- #
#+++++++++++++++++++++++++++++++++++++++++++++++++++++#
MYSQLLIB=
MYSQLLIB=`whereis libmysqlclient.so.*|grep -c -e"12"|awk '{print $0}'`
echo MYSQLLIB = $MYSQLLIB >> $LOGFILE
if [[ $MYSQLLIB1>0 ]];then
	echo "libmysqlclient.so.12 finns! MYSQLLIB="$MYSQLLIB >> $LOGFILE
	exitcode=0
fi
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++#
# -- Skapa en länk från libmysqlclient.so.12 till   --#
# -- libmysqlclient.so.14			    --#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++#
if test $MYSQLLIB = 0;then
	echo "Länkar libmysqlclient.so.12 till libmysqlclient.so.14" >> $LOGFILE
	ln -s libmysqlclient.so.14 libmysqlclient.so.12 >& $LOGFILE
fi
...
sed "s/'ADMIN'/'"$namn"'/g" $OLFIX_SQL/CreateOLFIX_Update.sql > $HOME/tmp/CreateOLFIXNy.tmp
