...
void frmLoadPrices::updatePrices()
{
//    bool load=FALSE;
    QListViewItem * item;
    QStringList lines;
    ...
    int recordsize;
    QString delemit=";";
    QString tmpfilename="/home/jan/tmp/pricefile.tmp";
//    QString loadfile="/home/jan/tmp/loadfile.sql";
    QFile pricefile( pricefilename );
    QFile tempfile(tmpfilename);
    ulong offset=0;
...
//    qDebug("tmpfilename=%s",tmpfilename.latin1());
	...
	if(i1a == 0){
		...
		if (prislista=="A"){
		    record=delemit+artnr+delemit+pris1+delemit+pris2+delemit+pris3+delemit+pris4+delemit+pris5+delemit+"\n";
		}
		if (prislista=="1"){
		    record=delemit+artnr+delemit+pris1+delemit+"\n";
		}
		if (prislista=="2"){
		    record=delemit+artnr+delemit+pris2+delemit+"\n";
		}
		if (prislista=="3"){
		    record=delemit+artnr+delemit+pris3+delemit+"\n";
		}
		if (prislista=="4"){
		    record=delemit+artnr+delemit+pris4+delemit+"\n";
		}
		if (prislista=="5"){
		    record=delemit+artnr+delemit+pris5+delemit+"\n";
		}
		if (prislista=="F"){
		    record=delemit+artnr+delemit+fpris+delemit+"\n";
		}				
...
//	qDebug("line=%s",line.latin1());
	...
	if(i1a == 0){
		...
		qDebug("Laddnings körning!");
		load=TRUE;
	    }
//	    qDebug("load=%d",load);
//	    qDebug("artnr=%s fpris=%s pris1=%s  pris2=%s  pris3=%s  pris4=%s  pris5=%s",artnr.latin1(),fpris.latin1(),pris1.latin1(),pris2.latin1(), pris3.latin1(),pris4.latin1(),pris5.latin1());
	}
            lines += line;
        }
    }
