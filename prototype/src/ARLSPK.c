...
/*
	...
	Kommando: ./ARLSPK produktklass

	Function: Lista artiklar för angiven produktklass, ARTIKELNR ARBENEMNING1 ARBENEMNING2 ur ARTIKELREG.
...
/*  	fprintf(stdout,"ARLSPK_Connection success\n");	*/
  	res = mysql_query(&my_connection,temp5);
/*  	fprintf(stderr,"ARLSPK hit res=%d\n",res);	*/
  	if (res){
/*  		fprintf(stderr,"ARLSPK ERROR\n");	*/
		fprintf(stderr,"Error: ARLSPK SELECT errno: %d\n",mysql_errno(&my_connection));
        }else{
		res_ptr=mysql_store_result(&my_connection);
		if (res_ptr){
			i=1;
/*			fprintf(stdout,"ARLSPK:Retrieved %lu rows\n",(unsigned long)mysql_num_rows(res_ptr));	*/
			fprintf(stdout,"OK: NR_%lu_:",(unsigned long)mysql_num_rows(res_ptr));
			while ((sqlrow=mysql_fetch_row(res_ptr)))  {
/*				fprintf(stderr,"ARLSPK:Fetched data....}");	*/
				display_row();
				i++;
			}
			fprintf(stdout,"\n");
