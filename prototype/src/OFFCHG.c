/***************************************************************************
                          OFFCHG.c  -  description
                             -------------------
			     Uppdatera/Ändra OFFERTREG från offert.
			     Offerthuvud.
    Version		 : 0.1
...
Fältavskiljare = _:_
	...
	UPDATE OFFERTREG SET KUNDNAMN="Nya Test AB
...
*/
 /*@unused@*/ static char RCS_id[] =
    "@(#) $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/olfix/Repository/prototype/src/OFFCHG.c,v 1.1 2006/11/05 07:29:57 janpihlgren Exp $ " ;
...
int main(int argc, char *argv[], char *envp[])
{
  ...
  char temp1a[]="UPDATE OFFERTREG SET KUNDNAMN=\"";
...
/* ================================================================================ */
  ...
  if (argv[1] != NULL){
  	strncpy(offertdata,argv[1],sizeof(offertdata));	/* 2005-02-23	*/
  }
  else{
  	fprintf(stderr,"Error: OFFCHG: Ange offertnummer!\n");
	exit(-1);
  }
...
//  	strncat(temp5,pos1,tmp);
        if (i == 0){
		strncpy(offertnr,pos1,tmp);
//		fprintf(stderr,"offertnr=%s\n",offertnr);
	}
        if (i == 1){
...
//	fprintf(stderr,"temp5[%d] = %s\n",i,temp5);	
  ...
  strncat(temp5,offertnr,sizeof offertnr);
...
/*  	fprintf(stderr,"ORDCHG:Connection success\n");	*/
  ...
  if (!res){
 	fprintf(stdout,"OK: OFFCHG Inserted %lu rows\n",
		(unsigned long)mysql_affected_rows(&my_connection));
        }else{
	fprintf(stderr,"Error: OFFCHG INSERT error: %d  %s\n", mysql_errno(&my_connection),
					mysql_error(&my_connection));
	}
	mysql_close(&my_connection);
 }else {
	fprintf(stderr,"Error: OFFCHG Connection failed\n");
 	if (mysql_errno(&my_connection))   {
 		fprintf(stderr,"Error: OFFCHG Connection error %d:  %s\n",
