...
*/
 /*@unused@*/ static char RCS_id[] =
    "@(#) $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/olfix/Repository/prototype/src/KTOCHG.c,v 1.1 2003/05/08 08:54:08 frazze Exp $ " ;
...
//  	fprintf(stderr,"KTOVIEW:Connection success\n");
  ...
  if (!res){
 	fprintf(stdout,"OK: KTOCHG_Inserted %lu rows\n",
		(unsigned long)mysql_affected_rows(&my_connection));
        }else{
/*	fprintf(stderr,"stderr-KTOCHG:INSERT error: %d  %s\n", mysql_errno(&my_connection),
					mysql_error(&my_connection));
*/
	fprintf(stderr,"Error: KTOCHG_INSERT error: %d  %s\n", mysql_errno(&my_connection),
					mysql_error(&my_connection));
	}
	mysql_close(&my_connection);
 }else {
	fprintf(stderr,"Error: KTOCHG_Connection failed\n");
 	if (mysql_errno(&my_connection))   {
 		fprintf(stderr,"Error: KTOCHG_Connection error %d:  %s\n",
