...
//    QDateTime dt = QDateTime::currentDateTime();
    ...
    frmDelOffert::listViewRader_format();
    frmDelOffert::getOffertLista();
    listViewOffert->setFocus();
//    lineEditOffertNbr->setFocus();
}


void frmDelOffert::slotPickupoffertnr( QListViewItem * item)
{
...
void frmDelOffert::lineEditOffertNbr_returnPressed()
{
    
    offertnr=lineEditOffertNbr->text();
    listViewRader->clear();
    frmDelOffert::getOfferthuvud();
}

void frmDelOffert::getOffertLista()
{
	...
	frmDelOffert::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(slotDataOnStdout() ) );
	frmDelOffert::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(slotDataOnStderr() ) );
	frmDelOffert::connect( process, SIGNAL(processExited() ),this, SLOT(slotOffertListaEndOfProcess() ) );
...
void frmDelOffert::slotOffertListaEndOfProcess()
{
    QListViewItem* item;
    int i;
    ...
    i = errorrad.find( QRegExp("Error:"), 0 );
         if (i != -1) {
	QMessageBox::critical( this, "DELOFFW",
		"ERROR!\n"+errorrad
	);
	errorrad="";
	i = -1;
    ...
    i = inrad.find( QRegExp("OK: NR_0_"), 0 );
         if (i != -1) {
	QMessageBox::information( this, "DELOFFW",
...
void frmDelOffert::getOfferthuvud()
{
	const char *userp = getenv("USER");
	QString usr(userp);
	inrad="";
	...
	frmDelOffert::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(slotDataOnStdout() ) );
	frmDelOffert::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(slotDataOnStderr() ) );
	frmDelOffert::connect( process, SIGNAL(processExited() ),this, SLOT(slotofferthuvudEndOfProcess() ) );
...
void frmDelOffert::slotofferthuvudEndOfProcess()
{
    int i,m;
...
*/
	...
	i = -1;
     ...
     frmDelOffert::getoffertrader();
}

void frmDelOffert::listViewRader_format()
{
...
void frmDelOffert::pushBtnHelp_clicked()
{
	inrad="";
	frmDelOffert::readResursFil();		// Hämta path till hjälpfilen
...
void frmDelOffert::pushButtonRadera_clicked()
{
    	const char *userp = getenv("USER");
	QString usr(userp);
	 inrad="";
	...
	process->addArgument(offertnr);	// offertnummer som ska raderas

	frmDelOffert::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(slotDataOnStdout() ) );
	frmDelOffert::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(slotDataOnStderr() ) );
	frmDelOffert::connect( process, SIGNAL(processExited() ),this, SLOT(slotOffertRaderaEndOfProcess() ) );
...
void frmDelOffert::readResursFil()
{
    /*****************************************************/
...
void frmDelOffert::clearKundInfo()
{
...
void frmDelOffert::slotDataOnStdout()
{
...
void frmDelOffert::slotDataOnStderr()
{
...
void frmDelOffert::getoffertrader()
{
	const char *userp = getenv("USER");
            QString usr(userp);
	 inrad4="";
	...
	frmDelOffert::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(slotoffertRadDataOnStdout() ) );
	frmDelOffert::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(slotoffertRadDataOnStderr() ) );
	frmDelOffert::connect( process, SIGNAL(processExited() ),this, SLOT(slotoffertRaderEndOfProcess() ) );
...
void frmDelOffert::slotoffertRaderEndOfProcess()
{
    /*  offertradrad	 */	
...
void frmDelOffert::slotoffertRadDataOnStdout()
{
     while (process->canReadLineStdout() ) {
	QString line = process->readStdout();
	inrad4.append(line);
...
void frmDelOffert::slotoffertRadDataOnStderr()
{
      while (process->canReadLineStderr() ) {
	QString line = process->readStderr();
...
void frmDelOffert::slotOffertRaderaEndOfProcess()
{
    int i = -1;
    i = errorrad.find( QRegExp("Deleted"), 0 );
         if (i != -1) {
	QMessageBox::warning( this, "DELOFFW",
		"Ingen post raderad!\n"
	);
	i = -1;
    }else{
