...
//	qDebug("inrad=%s",inrad.latin1()); 
    ...
    if (i != -1) {
	QMessageBox::information( this, "LSTARW","Betalningsvillkorregistret inneh�ller inga poster!\n");
	i = -1;
	exit(-1);
    }else{	 
	tmppek=tmp;
	...
	pos2=strstr(tmp,"_:");
	i=pos2-pos1;
	m=i+2;		// startposition f�r artikelnr
	k=0;
	for (j=7;j<i;j++){
	    antrad[k]=tmp[j];
	    k++;
...
//	qDebug("j=%d",j);
	for (k = 0;k < ant; k++){			// g� igenom alla raderna / posterna
	    i = j;
