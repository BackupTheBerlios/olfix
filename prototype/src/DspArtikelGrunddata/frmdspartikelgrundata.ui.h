...
void frmDspArtikelGrunddata::init()
{
    frmDspArtikelGrunddata::lineEditLagerplats_2->setText(lagerplats);
    frmDspArtikelGrunddata::lineEditArtikelNr->setFocus();
}

void frmDspArtikelGrunddata::lineEditArtikelNr_returnPressed()
{
...
void frmDspArtikelGrunddata::getGrunddata()
{
/************************************************************************/
...
//            qDebug("Anropa ARDSP, artikelnr=%s",artikelnr.latin1());
	...
	frmDspArtikelGrunddata::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(slotDataOnStderr() ) );
	frmDspArtikelGrunddata::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(slotDataOnStdout() ) );
            frmDspArtikelGrunddata::connect( process, SIGNAL(processExited() ),this, SLOT(slotGetGrDataEndOfProcess() ) );
...
void frmDspArtikelGrunddata::getLagerdata()
{
/************************************************************************/
...
/************************************************************************/
	...
	frmDspArtikelGrunddata::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(slotDataOnStderr() ) );
	frmDspArtikelGrunddata::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(slotDataOnStdout() ) );
            frmDspArtikelGrunddata::connect( process, SIGNAL(processExited() ),this, SLOT(slotgetLaDataEndOfProcess() ) );
...
//    qDebug("inrad=%s",inrad.latin1());
	...
	if (i != -1) {
	 ...
	 if (i4 != -1){
  	 ...
  	 m=i6-i5;
	 if (i5 != -1){
	     fpris=inrad.mid(i5+3,m-45);
//	     lineEditEnhet->setText(fpris);	// Försäljningspros skall inte visas i detta program.
	 }
	 ...
	 if (i14 != -1){
	     ...
	     lineEditStruktur->setText(struktur);
	 }
    ...
    // ========================
   frmDspArtikelGrunddata::getLagerdata();
    }
}

void frmDspArtikelGrunddata::slotgetLaDataEndOfProcess()
{
    int i,m;
...
//		lineEditValuta_2->setText(valuta);		// Ska inte visas kär
	    }
    
	    m=i9-i8;
	    if (i8 != -1){
		seninkpris=inrad.mid(i8+3,m-5);
//		lineEdit???->setText(seninkpris);		// Ska inte visas kär
	    }
	    
	    m=i10-i9;
	    if (i9 != -1){
		ikvant0=inrad.mid(i9+3,m-5);
//		lineEditSenInkopsKvantitet_2->setText(ikvant0);		// Ska inte visas kär
	    }
	    
	    m=i11-i10;
	    if (i10 != -1){
		ikvant1=inrad.mid(i10+3,m-5);
//		lineEditNestSenInkopsKvantitet_2->setText(ikvant1);		// Ska inte visas kär
	    }
	    
	    m=i12-i11;
	    if (i11 != -1){
		ikvant2=inrad.mid(i11+3,m-5);
//		lineEditNestNestSenInkopsKvantitet_2->setText(ikvant2);	// Ska inte visas kär
	    }
...
void frmDspArtikelGrunddata::slotDataOnStderr()
{
...
void frmDspArtikelGrunddata::slotDataOnStdout()
{
