...
***************************************************************************/
 /*@unused@*/ static char RCS_id[] =
    "@(#) $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/olfix/Repository/prototype/src/C/Attic/LEVLST.c,v 1.1 2004/05/03 04:42:57 janpihlgren Exp $ " ;
...
int main(int argc, char *argv[], char *envp[])
{
  ...
  char temp1[]="SELECT LEVNR,LEVNAMN FROM LEVREG ORDER BY LEVNAMN";
  char temp5[200]="";

/*  for (i=0;i< argc;i++){
  	fprintf(stderr,"LEVLST argv%d = %s\n",i,argv[i]);
  }
*/
...
/*  	fprintf(stdout,"LEVLST_Connection success\n");	*/
  	res = mysql_query(&my_connection,temp5);
/*  	fprintf(stderr,"LEVLST hit res=%d\n",res);	*/
  	if (res){
/*  		fprintf(stderr,"LEVLST ERROR\n");	*/
		fprintf(stderr,"Error: LEVLST SELECT errno: %d\n",mysql_errno(&my_connection));
        }else{
		res_ptr=mysql_store_result(&my_connection);
		if (res_ptr){
			i=1;
/*			fprintf(stdout,"LEVLST:Retrieved %lu rows\n",(unsigned long)mysql_num_rows(res_ptr));	*/
			fprintf(stdout,"OK: NR_%lu_:",(unsigned long)mysql_num_rows(res_ptr));
			while ((sqlrow=mysql_fetch_row(res_ptr)))  {
		 		//printf("LEVLST:Fetched data....}");
				display_row();
				i++;
			}
			fprintf(stdout,"\n");
