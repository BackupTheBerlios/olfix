/***************************************************************************
                          USERLST.c  -  description
                             -------------------
    Version		 			: 0.3x
    begin               : tis  29 okt  2002
    modified		 		: Sön  25 nov  2007
    copyright           : (C) 2002 by Jan Pihlgren
    email               : jan@pihlgren.se
    
    Testa anrop till databas på annan server/host
 ***************************************************************************/
...
/*
	INPUT:
	Function: gör  SELECT * FROM USR i databasen olfix

	OUTPUT: errornb och error (text)

*/
 /*@unused@*/ static char RCS_id[] =
    "@(#) $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/olfix/Repository/prototype/src/USERLST.c,v 1.6 2007/12/05 04:57:39 janpihlgren Exp $ " ;
...
/*  fprintf(stderr,"USERLST:Connection success\n");	*/
  ...
  res = mysql_query(&my_connection,"SELECT * FROM USR ORDER BY USERID");

  if (res){
	fprintf(stderr,"Error: USERLST_SELECT error: %s\n",mysql_error(&my_connection));
        }else{
	res_ptr=mysql_store_result(&my_connection);
	if (res_ptr){
		i=1;
/*		fprintf(stderr,"USERLST:Retrieved %lu rows\n",(unsigned long)mysql_num_rows(res_ptr));		*/
		fprintf(stdout,"NR_%lu_:",(unsigned long)mysql_num_rows(res_ptr));
		while ((sqlrow=mysql_fetch_row(res_ptr)))  {
/*			fprintf(stderr,"USERLST:Fetched data....}");	*/
/*			fprintf(stdout,"%d ",i);			*/
			display_row();
			i++;
		}
	if (mysql_errno(&my_connection))  {
		fprintf(stderr,"Error: USERLST_Retriev error:  %s\n", mysql_error(&my_connection));
		}
	}
	...
	mysql_close(&my_connection);
    	...
    	fprintf(stderr,"Error: USERLST_Connection failed\n");
    	if (mysql_errno(&my_connection))   {
    		fprintf(stderr,"Error: USERLST_Connection error %d:  %s\n",
