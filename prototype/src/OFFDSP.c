/***************************************************************************
                          OFFDSP.c  -  description
                             -------------------
			     Visa en offerts huvudpost
			     Ver: 0.4.3
    begin                : Fre  27  april 2006
    modified		 :
...
/*
	...
	Kommando: ./OFFDSP offertnr

	Function: Visa huvudpostens information på en kundorder i tabell ORDERREG

	OUTPUT: OFFERTNR ORDERNR KUNDNR ORDERTYP ORDERSTATUS ORDERDATUM LEVDATUM
	...
	FÖLJESEDEL FRAKTAVG SKATTEKOD MOMS VALUTA EXPORTKOD SPRAKKOD ORDERSUMMA
...
***************************************************************************/
 /*@unused@*/ static char RCS_id[] =
    "@(#) $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/olfix/Repository/prototype/src/OFFDSP.c,v 1.1 2006/04/27 18:08:59 janpihlgren Exp $ " ;
...
/* ================================================================================ */
  ...
  char temp1[]="SELECT * FROM OFFERTREG WHERE (OFFERTNR = \"";
  char temp2[]="\"";
  char temp4[]=")";
  char temp5[200]="";
  char offertnr[11]="";
...
/*  int i;
  fprintf(stderr,"OFFDSP argc = %d\n",argc);
  for (i=0;i< argc;i++){
  	fprintf(stderr,"OFFDSP argv%d = %s\n",i,argv[i]);
  }
*/
  strncpy(offertnr,argv[1],sizeof(offertnr));		
  strncat(temp5,temp1,strlen(temp1));
/* SELECT * FROM LEVREG WHERE (offertnr = "	*/
  strncat(temp5,offertnr,strlen(offertnr));/* 1234 */
/* SELECT * FROM LEVREG WHERE (offertnr = "1234	*/
  strncat(temp5,temp2,strlen(temp2)); /*  "     */
/* SELECT * FROM LEVREG WHERE (offertnr = "1234"	*/
  strncat(temp5,temp4,strlen(temp4)); /*  )     */
/* SELECT * FROM LEVREG WHERE (offertnr = "1234")	*/
  strncat(temp5,"\n",1);
/*  fprintf(stderr,"OFFDSP temp5 = %s\n",temp5);	*/

  mysql_init(&my_connection);
  if (mysql_real_connect(&my_connection, "localhost",  "olfix", "olfix", databas, 0, NULL, 0)){
/*  	fprintf(stdout,"ORDDSP_Connection success\n");		*/
  	res = mysql_query(&my_connection,temp5);
/*  	fprintf(stderr,"OFFDSP hit res=%d\n",res);		*/
  	if (res){
/*  		fprintf(stderr,"OFFDSP ERROR\n");		*/
		fprintf(stderr,"Error: OFFDSP SELECT errno: %d\n",mysql_errno(&my_connection));
        }
	else{
...
/*		fprintf(stderr,"num_rows=%d\n",num_rows);	*/
		if(num_rows != 0){
			...
			fprintf(stderr,"Error: OFFDSP Data saknas:  %s\n", mysql_error(&my_connection));
		}
		if (mysql_errno(&my_connection))  {
			fprintf(stderr,"Error: OFFDSP Retriev error:  %s\n", mysql_error(&my_connection));
		}
	}
	mysql_free_result(res_ptr);
    	...
    	fprintf(stderr,"Error: OFFDSP Connection failed\n");
    	if (mysql_errno(&my_connection))   {
    		fprintf(stderr,"Error: OFFDSP Connection error %d:  %s\n",
