...
/*
	...
	OUTPUT:	14 st fält,
		...
		ARIKVANT2, ARKALKPRIS, ARBESTKVANT, ARBESTPUNKT
		samt errornb och error (text)

***************************************************************************/
 /*@unused@*/ static char RCS_id[] =
    "@(#) $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/olfix/Repository/prototype/src/ARDSPL.c,v 1.1 2003/11/15 10:09:35 janpihlgren Exp $ " ;
...
/* ================================================================================ */
  ...
  if (argc <3){
  	fprintf(stderr,"Error: Artikelnr eller lagerställe saknas!\n");
	exit(-1);
  }
  for (i=0;i<argc;i++){
/*  	fprintf(stderr,"argc=%d argv[%d]=%s\n",argc,i,argv[i]);		*/
  ...
  char temp1[]="SELECT * FROM LAGERSTELLEREG WHERE (ARTIKELNR = \"";
  char temp1b[]=" AND ARLAGST = \"";
  char temp2[]="\"";
  char temp4[]=")";
  char temp5[200]="";
  char artikelnr[11]="";
...
/*  for (i=0;i< argc;i++){
  	fprintf(stderr,"ARDSPL main argv%d = %s\n",i,argv[i]);
  }
*/
  strncpy(lagerstelle,argv[1],strlen(argv[1]));
  strncpy(artikelnr,argv[2],strlen(argv[2]));

  strncpy(temp5,temp1,strlen(temp1));
/* SELECT * FROM LAGERSTELLEREG WHERE (artikelnr = "	*/
  strncat(temp5,artikelnr,strlen(artikelnr));/* 12334 */
/* SELECT * FROM LAGERSTELLEREG WHERE (artikelnr = "12334	*/
  strncat(temp5,temp2,strlen(temp2)); /*  "     */
/* SELECT * FROM LAGERSTELLEREG WHERE (artikelnr = "12334"	*/
  strncat(temp5,temp1b,strlen(temp1b));
/* SELECT * FROM LAGERSTELLEREG WHERE (artikelnr = "12334" AND LAGST = "	*/
  strncat(temp5,lagerstelle,strlen(lagerstelle));
/* SELECT * FROM LAGERSTELLEREG WHERE (artikelnr = "12334" AND LAGST = "1	*/
  strncat(temp5,temp2,strlen(temp2)); /*  "     */
/* SELECT * FROM LAGERSTELLEREG WHERE (artikelnr = "12334" AND LAGST = "1"	*/
  strncat(temp5,temp4,strlen(temp4)); /*  )     */
/* SELECT * FROM LAGERSTELLEREG WHERE (artikelnr = "12334" AND LAGST = "1")	*/
/*  strncat(temp5,"\n",1);				*/
/*  fprintf(stderr,"ARDSPL temp5 = %s\n",temp5);	*/

  mysql_init(&my_connection);
  if (mysql_real_connect(&my_connection, "localhost",  "olfix", "olfix", databas, 0, NULL, 0)){
/*  	fprintf(stdout,"ARDSPL_Connection success\n");	*/
  	res = mysql_query(&my_connection,temp5);
  	if (res){
/*  		fprintf(stderr,"ARDSPL ERROR\n");	*/
		fprintf(stderr,"Error: ARDSPL SELECT errno: %d\n",mysql_errno(&my_connection));
        }
	else{
...
*/
			...
			fprintf(stderr,"Error: ARDSPL Data saknas:  %s\n", mysql_error(&my_connection));
		}
		if (mysql_errno(&my_connection))  {
		fprintf(stderr,"Error: ARDSPL Retriev error:  %s\n", mysql_error(&my_connection));
		}
	}
	mysql_free_result(res_ptr);
    	...
    	fprintf(stderr,"Error: ARDSPL Connection failed\n");
    	if (mysql_errno(&my_connection))   {
    		fprintf(stderr,"Error: ARDSPL Connection error %d:  %s\n",
