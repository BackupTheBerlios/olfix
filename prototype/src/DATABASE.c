...
/*
	...
	val = 1; Skapa temporärfilen OLFIXpid.tmp
		Function: skapar en temporärfil av komandot "ps -C OLFIXW > path/tmp/OLFIXpid.tmp"
...
//		fprintf(stderr,"Error: DATABASE SELECT errno: %d\n",mysql_errno(&my_connection));
		if (mysql_errno(&my_connection) == 1146){
			fprintf(stderr,"Error: DATABASE. Databasen \"%s\" är inte en olfixdatabas!\n",databas);
		}else{
			fprintf(stderr,"Error: DATABASE SELECT errno: %d\n",mysql_errno(&my_connection));
		} 
	}else{
