...
*****************************************************************************/
/***************************************************************************
                          PLORDW  -  Skapa plocklista för kundorder
                             -------------------
		     version 0.1
    begin     	: Fre 31 mars 2005
    modified	: 
...
void frmPrtPlKundOrder::init()
{
    //      Hämta företagsnamn
    frmPrtPlKundOrder::getForetagsdata("FNAMN");
    //	Hämta sökvägen till kugar template. Default /usr/local/olfix/report
    frmPrtPlKundOrder::GetReportDir();
    frmPrtPlKundOrder::KugarVersion();
    lineEditOrderNr->setFocus();
}

void frmPrtPlKundOrder::lineEditOrderNr_returnPressed()
{
...
{
    frmPrtPlKundOrder::GetOrderHeader();
}

void frmPrtPlKundOrder::CreateReportHeader()
{
    int i;
    QString rad[59];
    QString rapportrad;

    QFile filnamn("/tmp/KuOrderPlocklista.kud");
    QTextStream stream(&filnamn);

    FileRemove("KuOrderPlocklista.kud");	// radera gammal fil.
...
//    rad[26]="      godsmerke CDATA #REQUIRED\n";
//    rad[27]="      moms CDATA #REQUIRED\n";
...
//    rad[32]="      varref CDATA #REQUIRED\n";
    ...
    rad[58].append("KuOrderPlocklista.kut\">\n");	// ange rätt template, absolut path
...
void frmPrtPlKundOrder::GetOrderHeader()
{
/************************************************************************/
...
/************************************************************************/
	...
	frmPrtPlKundOrder::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(DataOnStderr() ) );
            frmPrtPlKundOrder::connect( process, SIGNAL(processExited() ),this, SLOT(OrderHuvudEndOfProcess() ) );
...
void frmPrtPlKundOrder::GetOrderRow()
{
/************************************************************************/
...
/************************************************************************/
	...
	frmPrtPlKundOrder::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(DataOnStdout() ) );
	frmPrtPlKundOrder::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(DataOnStderr() ) );
            frmPrtPlKundOrder::connect( process, SIGNAL(processExited() ),this, SLOT(RaderEndOfProcess() ) );
...
void frmPrtPlKundOrder::OrderHuvudEndOfProcess()
{
    int i,m;
    i = -1;
...
//	Skapa orderhuvud
//********************************************************        
    frmPrtPlKundOrder::CreateReportHeader();
//********************************************************    
//	Hämta orderrader    
//********************************************************    
    frmPrtPlKundOrder::GetOrderRow();
}

void frmPrtPlKundOrder::RaderEndOfProcess()
{
     QString orderradnr;
...
void frmPrtPlKundOrder::CreateReportFot()
{
    QFile filnamn("/tmp/KuOrderPlocklista.kud");
    QTextStream stream(&filnamn);
    QString plockrad;	// beställningsrad

    filnamn.open( IO_WriteOnly | IO_Append );
    
    plockrad="<txt level=\"2\" ";
    plockrad.append(" ordertext=\"");
    plockrad.append(besttext);
    plockrad.append("\"/>\n");
    plockrad.append("</KugarData>");
    
    stream << plockrad;
    filnamn.close();
    frmPrtPlKundOrder::CallKugar();
}

void frmPrtPlKundOrder::DataOnStdout()
{
...
void frmPrtPlKundOrder::DataOnStderr()
{
...
void frmPrtPlKundOrder::GetReportDir()
{
    QString hemkatalog;
...
void frmPrtPlKundOrder::FileRemove(QString filnamn)
{
//  Ta bort temporärfilen /tmp/KuOrderPlocklista.kud eller filen /tmp/Bestellning.txt
    bool status;
    QDir d = QDir::root();                   // "/"
    if ( !d.cd("tmp") ) {                       // "/tmp"
...
void frmPrtPlKundOrder::KugarVersion()
{
...
//   qDebug("kugarversion=%s",kugarversion.latin1());
   frmPrtPlKundOrder::FileRemove("kugarversion.txt");
}

void frmPrtPlKundOrder::CallKugar()
{
    if (kugarversion<"1.2.92"){
	kommando="kugar -d /tmp/KuOrderPlocklista.kud -r "+reportpath+"KuOrderPlocklista.kut";
...
/************************************************************************/
	...
	frmPrtPlKundOrder::connect( process, SIGNAL(readyReadStdout() ),this, SLOT(DataOnStdout() ) );
	frmPrtPlKundOrder::connect( process, SIGNAL(readyReadStderr() ),this, SLOT(DataOnStderr() ) );
            frmPrtPlKundOrder::connect( process, SIGNAL(processExited() ),this, SLOT(ForetagsdataEndOfProcess() ) );
...
void frmPrtPlKundOrder::ForetagsdataEndOfProcess()
{
    int i,m;
    QString posttyp="";
    i = -1;
